    ...
    return (
      <article className="mx-auto max-w-2xl px-5 py-10 pb-16 font-sans leading-relaxed break-words text-foreground">
        ...
        <p className="mb-2">
          ...
          backend API via an HTTP POST request body (not in the URL query string)
          and then to Google’s Gemini API to generate a reply. The active UI
          language may be sent as an <code>Accept-Language</code> header and
          <code>locale</code> query parameter so replies can match your language.
        </p>
        ...
        <ul className="mb-4 list-disc space-y-1 pl-5">
          <li>
            Theme preference (<code>light</code> / <code>dark</code> /{" "}
            <code>system</code>)
          </li>
          <li>
            Optional chat save preference (<code>chat_save_opt_in_v1</code>)
          </li>
          <li>
            Recent chat messages (<code>chat_state_v1</code>) — only if you
            explicitly enable “Save chat on this device”
          </li>
        </ul>
        <p className="mb-2">
          ...
          Chat history is <strong>not</strong> saved by default; if you opt in,
          messages are stored only in your browser so the conversation can survive
          refresh. It is not uploaded as a saved account history on my servers.
          You can clear saved chat from the chat UI. Prompts you send are still
          transmitted to the chatbot API as described in section 3.2. Draft input
          is not persisted.
        </p>
        <p className="mb-4">
          <strong>Purpose:</strong> Convenience (theme; optional chat restore).
          <br />
          <strong>Legal basis (GDPR):</strong> Art. 6(1)(a) — consent for optional
          chat persistence; Art. 6(1)(f) and/or Art. 6(1)(a) for theme preference
          as a non-essential UX preference.
        </p>
        ...
        <div className="mb-4 overflow-x-auto">
          <table className="w-full border-collapse text-left text-sm">
            <thead>
              <tr>
                <th className="border border-border bg-surface p-2">
                  Recipient
                </th>
                <th className="border border-border bg-surface p-2">
                  Role
